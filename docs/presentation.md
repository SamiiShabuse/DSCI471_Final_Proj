...
- Selected after v1-to-v5 ablations (pretrained text encoder = largest gain).
...
*Chart: 2x2 metric bars from `04_final_results.ipynb`.*
...
1. **Modality gap** - text-to-image vs TF-IDF text-to-text (with rich JSON descriptions).
2. **Low Top-1 overall** - both models struggle on shopper/short queries (~5-8%).
3. **Synthetic queries** - templated/shopper generators are not real search logs.
...
**Conclusion:** Dual-encoder improved through v1-to-v4 (MiniLM text encoder critical), but **keyword search wins Top-1** when the gallery is fully text-indexed. Cross-modal retrieval is viable and wins on some Top-5 / MRR metrics.
...
|---|---|
| v1-to-v5 experiment notebooks | Unified `src/` pipeline |
